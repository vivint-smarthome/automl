...
"""
Example usage:
    "$PYTHONPATH" python dataset/inspect_tfrecords.py \
      --file_pattern gs://arrtest/tfrecords/test/* \
      --model_name "efficientdet-lite2" \
      --samples 10 --save_samples_dir ../../train_samples/ \
      -hparams="label_map.1='person',label_map.2='Animal',label_map.3='face',label_map.4='automobile',label_map.5='package'"
"""
  ...
  def visualize(self):
    ...
    for i, zip_data in enumerate(zip(gt_data, images, scales)):
      ...
      if boxes.shape[0] > 0:
        ...
        try:
            category_index = {idx: {'id': idx, 'name': self.cls_to_label[str(idx)]} 
                              # changed to str(idx) instead of idx, not sure if this is a bug?
