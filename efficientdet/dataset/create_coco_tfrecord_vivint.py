...
Example usage:
    PYTHONPATH=".:$PYTHONPATH" python dataset/create_coco_tfrecord_vivint.py --logtostderr \
                    --object_annotations_file=gs://arrtest/mscoco_test.json \
                    --output_file_prefix=gs://amey_model_train/coco_tfrecords/test_ \
                    --num_shards=100
                    --num_threads=4
"""
...
def get_vivint_category(catId):
    """
    Custom function which maps category ids to continous numbers starting from 1 to # of categories
    """
    # label_map: {1:'person',2:'Animal',3:'face',4:'automobile',5:'package'}
    mapping = {1:1, 578:2, 614:3, 615:4, 621:5}
...
def create_tf_example(image,
  ...
  # vivint custom params
  # edit for normalized COCO json format
  ...
  feature_dict = {
      ...
      'image/filename':
          tfrecord_util.bytes_feature(gs_url.encode('utf8')),
      'image/source_id':
  ...
  if bbox_annotations:
    ...
    for object_annotations in bbox_annotations:
      ...
      if width <= 0 or height <= 0:
        ...
        continue
      if x + width > 1 or y + height > 1: # Normalized bbox
        num_annotations_skipped += 1
        continue
      # Normalized bbox -->
      xmin.append(float(x))
      ...
      category_ids.append(get_vivint_category(category_id)) #changed code to get the carrect mapping of the category
